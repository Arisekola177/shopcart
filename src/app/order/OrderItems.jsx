  ...
  return (
    <div className='grid grid-cols-5 text-xs md:text-sm gap-4 border-t-[1.5px] border-slate-200 py-4 items-center'>
         <div className='col-span-2 justify-self-start flex gap-2 md:gap-4'>
           ...
           <div className='justify-self-center'><FormattedPrice amount={item.amount} /></div>
           <div className='justify-self-center'> {item.quantity}</div>
           <div className='justify-self-end font-semibold'> ${(item.quantity * item.amount ).toFixed(2)}</div>
     </div>
  )
}
